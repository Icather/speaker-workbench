...
- **The bottom of the page was unreachable at most window widths.** `main` was sized with `height:calc(100vh - 45px)` — a hard-coded guess at the header's height. But the header is `flex-wrap`, so its real height moves with the window: measured **46px at 1600px wide, 84px at 1280, 85px at 900, 123px at 760**. Whenever it exceeded 45px, `main` overflowed the viewport and `body{overflow:hidden}` made the excess permanently unreachable — no scrollbar could bring it back. Fixed by making `body` a column flex container (`header` takes its natural height, `main` takes the rest) and deleting the `100vh` arithmetic. Verified at five widths: `main`'s bottom edge now lands exactly on the viewport height in all of them. The 1600px coincidence is why this hid for so long — at that width the guess happened to be right.
